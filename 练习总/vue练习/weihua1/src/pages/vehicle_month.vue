  ...
  export default {
    ...
    methods:{
      search(){
        ...
        post('/DCMonth', data, function(res) {
          if (res.code == 0) {
            for (let msg in res.data.data) {
              ...
              that.columns1[0].width = 60
              that.columns1[1].key = 'name'
              that.columns1[1].width = 145
              that.columns1[1].className='tab_sty'
              that.columns1[2].key = 'install_car'
              that.columns1[2].width = 120
              that.columns1[3].key = 'location_car'
              that.columns1[3].width = 120
              that.columns1[4].key = 'out_online_car'
              that.columns1[4].width = 120
              that.columns1[5].key = 'online'
              that.columns1[5].width = 80
              that.columns1[6].key = 'this_Speeding'
              that.columns1[6].width = 145
              that.columns1[7].key = 'last_Speeding'
              that.columns1[7].width = 145
              that.columns1[8].key = 'percentage'
            }
            that.loading = false
          } else {
    ...
    computed:{
      exports:function () {
        ...
        return exportMonth(y,m,'DCMonth')
      }
    }
  }
</script>
