  ...
  <div class="container">
    ...
    <ul>
      <li class="list-item" v-for="(item,index) in list " data-type="0">
        <div class="list-box" @touchstart.capture="touchStart" @touchend.capture="touchEnd" @touchmove.capture="touchMove" @click="skip">
...
<script>
  ...
  export default{
    ...
    methods : {
      ...
      touchMove(e){
        var that = this;
        that.sX1.push(e.touches[0].clientX)
        if(that.sX1[0] <= that.sX){
          that.show = true;
        }
      },
      //删除
